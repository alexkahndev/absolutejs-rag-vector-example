...
type CreateRagDemoStateOptions = {
  ...
  ragBackends: {
    ...
    list: () => Array<{ id: DemoBackendMode; label: string; path: string; available: boolean; reason?: string }>;
...
const buildEmailSyncDocuments = async ({
  ...
  const attachmentUploads = listed.messages.flatMap((message) =>
    (message.attachments ?? []).map((attachment, index) => ({
      ...
      title:
        attachment.title ?? `${message.subject ?? message.id} · ${attachment.name}`,
    })),
  );
    ...
    documents: [
      ...listed.messages.map((message) => ({
        ...
        metadata: {
          ...
          messageId: message.id,
          receivedAt: typeof message.receivedAt === "string" ? message.receivedAt : undefined,
  ...
  loadRAGDocumentsFromUploads({
    uploads: await Promise.all(
      keys.map(async (key) => {
        ...
        const bytes = await Bun.file(join(process.cwd(), "rag-demo-corpus", relativePath)).bytes();
        return {
...
export const createRagDemoState = ({ ragDb, ragBackends }: CreateRagDemoStateOptions) => {
  ...
  const gmailMode: DemoEmailSyncMode = gmailAccessToken ? "live" : "fixture";
  ...
  const ensureTextColumn = (columnName: string, defaultValue: string) => {
    const columns = ragDb.prepare(`PRAGMA table_info(${RAG_DEMO_DOCUMENT_TABLE_NAME})`).all() as Array<{ name?: unknown }>;
  ...
  const syncStatePath = join(process.cwd(), ".absolute", "rag-vector-demo-sync-state.json");
  ...
  const syncSourceDefinitions: DemoSyncSourceDefinition[] = [
    ...
    {
      id: "sync-storage",
      label: storageMode === "live" ? "Storage sync account" : "Storage sync fixture",
      kind: "storage",
      description:
        storageMode === "live"
      ...
      metadata: {
        ...
        persistence: syncStatePath,
        provider: storageEndpoint ? "S3-compatible storage" : "Amazon S3 / Cloudflare R2",
        schedule: "manual",
        liveReady:
          storageMode === "live"
    ...
    {
      id: "sync-email-gmail",
      label: gmailMode === "live" ? "Gmail sync account" : "Gmail sync fixture",
      kind: "email",
      description:
        gmailMode === "live"
  ...
  let syncSources: RAGSyncSourceRecord[] = syncSourceDefinitions.map((source) => ({
    id: source.id,
    label: source.label,
    kind: source.kind,
  ...
  const activeSyncSourceRuns = new Map<string, Promise<Awaited<ReturnType<typeof syncSourceById>>>>();
  let activeSyncAllRun: Promise<Awaited<ReturnType<typeof syncAllSourcesInternal>>> | null = null;
  ...
  const toContentFormat = (value: unknown): DemoContentFormat =>
    value === "markdown" || value === "html" || value === "text" ? value : "text";

  const toChunkingStrategy = (value: unknown): DemoChunkingStrategy =>
    value === "paragraphs" || value === "sentences" || value === "fixed" || value === "source_aware"
      ? value
      : "paragraphs";
  ...
  const resolveRequestedBackendMode = (
    ...
    if (isBackendMode(cookieValue) && ragBackends.backends[cookieValue].available) {
      return cookieValue;
    }
  ...
  const normalizeDocumentRow = (row: Record<string, unknown>): DemoDocument => {
    ...
    const chunkCount = toNumber(
      row.chunk_count,
      countPreparedChunks({ id, text, title, source, format, chunkStrategy, kind }, kind),
    );
  ...
  const fetchDocumentRows = (kind?: RagDocumentKind) => {
    const rows =
      kind === "seed" || kind === "custom"
        ? (ragDb
            .prepare(`${SELECT_DOCUMENT_SQL} WHERE kind = ? ORDER BY created_at ASC`)
  ...
  const upsertDocument = (document: {
    ...
    const chunkSize = document.chunkSize ??
      (document.kind === "seed" ? RAG_DEMO_DEFAULT_CHUNK_SIZE : RAG_DEMO_DEFAULT_CUSTOM_CHUNK_SIZE);
    const createdAt = document.createdAt === undefined ? now : toNumber(document.createdAt, now);
  ...
  const deleteDocumentRow = (id: string) => {
    ragDb.prepare(`DELETE FROM ${RAG_DEMO_DOCUMENT_TABLE_NAME} WHERE id = ?`).run(id);
  };

  const countPreparedChunksForSync = (document: RAGIngestDocument) =>
    countPreparedChunks(
      {
        id: document.id ?? hashValue(document.source ?? document.title ?? document.text).slice(0, 12),
        kind: "custom",
  ...
  const replaceSyncSourceRecord = (nextRecord: RAGSyncSourceRecord) => {
    syncSources = syncSources.map((source) => (source.id === nextRecord.id ? nextRecord : source));
  };

  const readSyncHistory = (metadata: Record<string, unknown> | undefined): DemoSyncRunHistoryEntry[] => {
    ...
    return value
      .filter((entry): entry is DemoSyncRunHistoryEntry => (
        !!entry
        && typeof entry === "object"
  ...
  const resolveSyncTrigger = (
    ...
    if (options?.background) {
      return typeof definition.metadata?.schedule === "string" ? "scheduled" : "background";
    }
  ...
  const ensureSyncSourcesLoaded = async () => {
    ...
    syncSources = syncSourceDefinitions.map((definition) => {
      const existing = persisted.find((record: RAGSyncSourceRecord) => record.id === definition.id);
      return {
        id: definition.id,
        ...
        metadata: buildSyncMetadata(definition, existing?.metadata),
  ...
  const syncSourceRecord = (id: string) => syncSources.find((source) => source.id === id) ?? null;

  const syncSourceById = async (id: string, options?: { background?: boolean }) => {
    ...
    replaceSyncSourceRecord({
      ...
      metadata: buildSyncMetadata(definition, currentRecord?.metadata, {
        lastTrigger: trigger,
      }),
    ...
    try {
      ...
      for (const document of existingDocuments) {
        const syncKey = typeof document.metadata.syncKey === "string" ? document.metadata.syncKey : document.source;
      ...
      for (const [index, document] of loaded.documents.entries()) {
        ...
        const documentId = existing?.id ?? `sync-${definition.id}-${hashValue(syncKey).slice(0, 12)}`;
        upsertDocument({
          id: documentId,
          kind: "custom",
          title: toSafeText(document.title, syncKey.split("/").at(-1) ?? documentId),
          source: syncKey,
          text: document.text,
          format: toContentFormat(document.format),
          chunkStrategy,
      ...
      for (const document of existingDocuments) {
        const syncKey =
          typeof document.metadata.syncKey === "string" ? document.metadata.syncKey : document.source;
  ...
  const queueSyncSource = async (id: string, options?: { background?: boolean }) => {
  ...
  const syncAllSourcesInternal = async (options?: { background?: boolean }) => {
    await ensureSyncSourcesLoaded();
  ...
  const queueSyncAllSources = async (options?: { background?: boolean }) => {
    if (!activeSyncAllRun) {
  ...
  const ensureSeedDocuments = async () => {
    const now = Date.now();
    const seedDocuments = (await getSeedDocuments()).map((doc: RagSeedDocument) => ({
      id: doc.id,
      kind: "seed" as const,
    ...
    if (seedIds.length > 0) {
      ...
      ragDb.prepare(`DELETE FROM ${RAG_DEMO_DOCUMENT_TABLE_NAME} WHERE kind = 'seed'`).run();
    }
  ...
  const buildCorpusFromDb = (): RagSeedDocument[] => fetchDocumentRows().map(toSeedDocument);
  ...
  const rebuildVectorStores = async () => {
    ...
    return Object.fromEntries(results) as Record<DemoBackendMode, BackendSeedStats>;
  };
  ...
  const createIndexManager = () => ({
    listDocuments: ({ kind }: { kind?: string } = {}) => getDocumentList({ kind }),
    ...
    syncSource: async (id: string, options?: { background?: boolean }) =>
      options?.background ? queueSyncSource(id, options) : syncSourceById(id, options),
    syncAllSources: async (options?: { background?: boolean }) =>
      options?.background ? queueSyncAllSources(options) : syncAllSourcesInternal(options),
    reseed: async () => {
    ...
    reset: async () => {
      ragDb.prepare(`DELETE FROM ${RAG_DEMO_DOCUMENT_TABLE_NAME} WHERE kind = 'custom'`).run();
      await ensureSeedDocuments();
      const status = await rebuildVectorStores();
  ...
  const normalizeSearchPayload = (body: unknown): RAGCollectionSearchParams | null => {
    ...
    const filter = Object.fromEntries([...filterEntries, ...compatFilterEntries]);

    return {
      query,
      topK: typeof payload.topK === "number" ? payload.topK : undefined,
      scoreThreshold:
        typeof payload.scoreThreshold === "number" ? payload.scoreThreshold : undefined,
  ...
  const listBackendDiagnostics = () =>
    ragBackends.list().map((backend) => {
      ...
      return {
        ...
        capabilities: runtime.available ? runtime.rag?.getCapabilities?.() : undefined,
      };
    });
  ...
  const initialize = async (): Promise<RagDemoStartup> => {
    ...
    const startupSeedTime = Date.now() - startupStart;
