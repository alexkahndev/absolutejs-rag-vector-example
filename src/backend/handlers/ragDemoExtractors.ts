...
const imageOCR = createRAGOCRProvider({
  ...
  extractText: (input) => {
    ...
    if (name.includes("receipt")) {
      return {
        text:
          "Receipt image OCR shows invoice INV-2048, customer Harbor Outfitters, and a fulfillment note that metadata tags must remain stable during ingest.",
        metadata: {
    ...
    return {
      text:
        "Image OCR extracted demo text from a seeded binary asset so the example can prove first-party OCR extraction without external services.",
      metadata: {
        extractedFrom: "demo-image",
...
const pdfOCR = createRAGOCRProvider({
  ...
  extractText: (input) => {
    ...
    if (name.includes("scanned-receipt")) {
      return {
        text:
          "Scanned PDF OCR recovered invoice INV-7148, warehouse lane C3, and the note that the retrieval benchmark should cite files by page when available.",
        metadata: {
    ...
    return {
      text:
        "Demo PDF OCR fallback extracted seeded content from an image-only PDF so the example can show the first-party OCR path.",
      metadata: {
        extractedFrom: "demo-pdf",
        ocrEngine: "Absolute demo PDF OCR",
      },
        ...
        segments: [
          {
            ...
            startMs: 0,
            text:
              "At timestamp 00:00 to 00:07, the incident bridge says escalation context and caller roles should stay visible in retrieval evidence.",
          },
          {
            endMs: 16000,
            speaker: "Morgan",
            startMs: 7000,
            text:
              "At timestamp 00:07 to 00:16, the follow-up says grounded answers should preserve source labels and timestamp-aware references.",
          },
        ],
        ...
        segments: [
          {
            ...
            startMs: 0,
            text:
              "The first segment says sync health, admin jobs, and grounded answer diagnostics should stay together in the same operational surface.",
          },
          {
            endMs: 22000,
            speaker: "Narrator",
            startMs: 11000,
            text:
              "The second segment says scoped retrieval and citation review should remain directly connected to the document evidence they came from.",
          },
        ],
      ...
      segments: [
        {
          ...
          startMs: 0,
          text:
            "PDF, OCR, archive, spreadsheet, presentation, and legacy document ingest all land in the same retrieval surface.",
        },
        {
          endMs: 24000,
          speaker: "Narrator",
          startMs: 12000,
          text:
            "The follow-up segment says workflow streaming and evidence inspection should stay on the same React route as sync and admin controls.",
        },
      ],
