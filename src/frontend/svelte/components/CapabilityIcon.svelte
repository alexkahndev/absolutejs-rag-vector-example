<script lang="ts">
  ...
  let { capability, size = ICON_DEFAULT_SIZE }: CapabilityIconProps = $props();
  ...
  let icon = $derived(ICONS[capability]);
  let bgSize = $derived(size + ICON_BADGE_PADDING);
</script>

<span
  class="cap-badge"
  style="align-items:center;background:{icon.color}18;border-radius:4px;display:inline-flex;height:{bgSize}px;justify-content:center;width:{bgSize}px"
  title={icon.label}
>
  <svg
    fill="none"
