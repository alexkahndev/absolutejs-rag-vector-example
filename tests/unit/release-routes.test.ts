...
describe("release route smoke", () => {
  it("renders the HTMX release fragment with scenario controls and evidence drills", async () => {
    const response = await server.handle(
      new Request("http://absolute.local/demo/release/sqlite-native/htmx?workspace=alpha"),
    );
  ...
  it("returns the JSON release governance contract", async () => {
    await postJson(
      ...
      { actionId: "switch-to-blocked-multivector-scenario", workspace: "alpha" },
      false,
    );

    const response = await getJson("http://absolute.local/demo/release/sqlite-native?workspace=alpha");
    ...
    expect(body.releaseStatus?.retrievalComparisons?.readyToPromoteByLane).toBeArray();
    expect(body.releaseStatus?.retrievalComparisons?.adaptiveNativePlannerBenchmark?.suiteId).toBe("rag-native-planner-larger-corpus");
    expect(body.incidentStatus?.incidentClassificationSummary?.totalMultiVectorCount).toBeGreaterThan(0);
    expect(body.remediationStatus?.recentIncidentRemediationExecutions).toBeArray();
  });

  it("returns the beta workspace governance contract", async () => {
    const response = await getJson("http://absolute.local/demo/release/sqlite-native?workspace=beta");
    ...
    expect(body.incidentStatus?.incidentClassificationSummary?.totalMultiVectorCount).toBeGreaterThan(0);
    expect(body.incidentStatus?.incidentClassificationSummary?.totalGeneralCount).toBe(0);
  });
  ...
  it("runs the remediation drill through the real JSON workflow and persists execution history", async () => {
    ...
    expect(body.message).toBe("Ran the remediation drill through the published remediation execution workflows.");
    expect(body.release?.remediationStatus?.recentIncidentRemediationExecutions?.length).toBeGreaterThan(0);
    expect(body.release?.scenario?.id).toBe("blocked-multivector");

    const refreshed = await getJson("http://absolute.local/demo/release/sqlite-native?workspace=alpha");
    const refreshedBody = await refreshed.json() as Record<string, any>;

    expect(refreshed.status).toBe(200);
    expect(refreshedBody.remediationStatus?.recentIncidentRemediationExecutions?.length).toBeGreaterThan(0);
  });
  ...
  it("runs the incident drill through the real JSON workflow and persists incident history", async () => {
    ...
    expect(body.ok).toBe(true);
    expect(body.message).toBe("Ran the incident drill through the published incident workflows.");
    expect(body.release?.incidentStatus?.incidentSummary?.openCount).toBe(0);
    expect(body.release?.incidentStatus?.incidentSummary?.resolvedCount).toBeGreaterThan(0);
    expect(body.release?.incidentStatus?.incidentClassificationSummary?.totalMultiVectorCount).toBeGreaterThan(0);
    expect(body.release?.incidentStatus?.recentIncidents?.[0]?.status).toBe("resolved");

    const refreshed = await getJson("http://absolute.local/demo/release/sqlite-native?workspace=alpha");
    const refreshedBody = await refreshed.json() as Record<string, any>;

    expect(refreshed.status).toBe(200);
    expect(refreshedBody.incidentStatus?.incidentSummary?.resolvedCount).toBeGreaterThan(0);
    expect(refreshedBody.incidentStatus?.incidentClassificationSummary?.totalMultiVectorCount).toBeGreaterThan(0);
  });
  ...
  it("renders the multivector evidence drill through the real search workflow", async () => {
    ...
    expect(html).toContain("Winning evidence · guide/multivector-release-guide.md");
    expect(html).toContain("Late interaction · Launch checklist · collapsed 1 parent");
  ...
  it("runs the handoff completion drill through the real JSON workflow", async () => {
    ...
    expect(body.ok).toBe(true);
    expect(body.message).toBe("Ran the stable handoff completion drill through the published handoff workflows.");
    ...
    expect(body.release?.handoffStatus?.handoffs?.[0]?.readyForHandoff).toBe(true);
  ...
  it("runs the handoff incident drill through the real JSON workflow", async () => {
    ...
    expect(body.ok).toBe(true);
    expect(body.message).toBe("Ran the stale handoff incident drill through the published handoff incident workflows.");
    expect(body.release?.handoffStatus?.incidentSummary?.openCount).toBe(0);
    expect(body.release?.handoffStatus?.incidentSummary?.resolvedCount).toBeGreaterThan(0);
    expect(body.release?.handoffStatus?.incidents?.[0]?.status).toBe("resolved");
    expect(body.release?.handoffStatus?.recentHistory?.length).toBeGreaterThan(0);

    const refreshed = await getJson("http://absolute.local/demo/release/sqlite-native?workspace=alpha");
    const refreshedBody = await refreshed.json() as Record<string, any>;

    expect(refreshed.status).toBe(200);
    expect(refreshedBody.handoffStatus?.incidentSummary?.resolvedCount).toBeGreaterThan(0);
    expect(refreshedBody.handoffStatus?.recentHistory?.length).toBeGreaterThan(0);
  });
